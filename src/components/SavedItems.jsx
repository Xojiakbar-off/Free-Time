...
export default function SavedItems({ embedded = false }) {
  ...
  return (
    <section id="saved" className={embedded ? '' : 'max-w-7xl mx-auto px-4 py-12 scroll-mt-16'}>
      ...
      {bookmarks.length>0&&(
        <div className="mb-10">
          <h6 className="text-xs uppercase tracking-widest text-slate-500 dark:text-slate-400 font-semibold mb-3 flex items-center gap-2"><Bookmark size={13} className="text-indigo-500 dark:text-indigo-400"/>Bookmarks</h6>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {bookmarks.map(b=>(
              <div key={`${b.type}-${b.id}`} className="glass-panel rounded-2xl p-5 card-hover">
                ...
                <button onClick={()=>toggleBookmark({id:b.id,type:b.type})} className="mt-3 px-3 py-1.5 rounded-lg bg-red-500/20 text-red-600 dark:text-red-400 text-xs hover:bg-red-500/30 transition-colors"><Trash2 size={12} className="inline mr-1"/>Remove</button>
              </div>
            ))}
          </div>
        </div>
